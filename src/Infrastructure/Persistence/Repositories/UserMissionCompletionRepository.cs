...
internal sealed class UserMissionCompletionRepository(AppDbContext dbContext) : IUserMissionCompletionRepository
{
    private const string ActorName = "mission-reward";

    public async Task<IReadOnlyList<MissionCompletionReadModel>> GetCompletionsAsync(
        Guid userId,
        IReadOnlyCollection<string> periodKeys,
        CancellationToken cancellationToken = default)
    {
        return await dbContext.UserMissionCompletions
            .AsNoTracking()
            .Where(x => x.UserId == userId && periodKeys.Contains(x.PeriodKey))
            .Select(x => new MissionCompletionReadModel(
                x.MissionCode,
                x.PeriodKey,
                x.CompletedAt,
                x.RewardCredits))
    ...
    public async Task<bool> TryCompleteAndRewardAsync(
        Guid userId,
        string missionCode,
        string periodKey,
        long rewardCredits,
        ...
        CancellationToken cancellationToken = default)
    {
        // The completion row's unique (user, code, period) index is the idempotency key: insert it
        // first, and a concurrent duplicate loses with a unique-violation we treat as "already
        // completed". Only on a successful insert do we move money, all inside one transaction so a
        // credited wallet always has a matching ledger row.
        ...
        dbContext.UserMissionCompletions.Add(completion);
        ...
        {
            // Lost the race against a concurrent completion of the same (user, code, period).
            dbContext.Entry(completion).State = EntityState.Detached;
            await transaction.RollbackAsync(cancellationToken);
            return false;
        }

        await CreditWalletAsync(userId, rewardCredits, completion.Id, occurredAt, cancellationToken);
